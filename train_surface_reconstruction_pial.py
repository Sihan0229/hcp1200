...
def load_dataset(in_dir, surf_hemi='left'):
    ...
    for path in tqdm(subj_paths, desc='Loading data'):
        ...
        vert_pial_path = os.path.join(path, f"{subj_id}_hemi-{hemi}_vert_pial.pt")
        vert_wm_path = os.path.join(path, f"{subj_id}_hemi-{hemi}_vert_wm_after_smooth.pt")
        ...
        # 读取vol_in
        vert_pial = torch.load(vert_pial_path)
...
class SurfaceDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'vert_pial': item['vert_pial'].float(),
...
def chamfer_distance_naive(x, y):
    """
    Naive Chamfer distance (slow, CPU friendly)
    """
    x = x.squeeze(0)  # (N1, 3)
    y = y.squeeze(0)  # (N2, 3)
...
# =============模型训练==============
def train(model, dataloader, optimizer, device, init_face, out_dir, hemi, surface, num_epochs=100, n_steps=7):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            ...
            target_vert_pial = batch['vert_pial'].to(device)
            init_vert = batch['vert_wm'].to(device)
            init_face = init_face.to(device) 

            optimizer.zero_grad()
            # print('vol.shape: ', vol.shape)
            vol = vol.squeeze(2)  
            if init_vert.shape[1] == 1:
                init_vert = init_vert.squeeze(1)

            # print('vol.shape: ', vol.shape)
            # print('init_vert_pial.shape: ', init_vert.shape)
            # print('target_vert_pial.shape: ', target_vert_pial.shape)

            
            
            ## vert_pial = nn_surf_pial(vert_wm, vol_in, n_steps=7)
            pred_vert_pial = model(init_vert, vol, n_steps=n_steps)  # 模型输出
            # pred_vert_smoothed = taubin_smooth(pred_vert_wm, face, n_iters=5)  # 加平滑

            if target_vert_pial.shape[1] == 1:
                target_vert_pial = target_vert_pial.squeeze(1)
            ...
            # loss_mse = F.mse_loss(pred_vert_pial, target_vert_pial)
            loss_edge = edge_distortion(pred_vert_pial, target_vert_pial, edge)
            # loss_area = area_distortion(pred_vert_pial, target_vert_pial, init_face)
            sampled_pred = random_sample_points(pred_vert_pial, 5000)
            sampled_target = random_sample_points(target_vert_pial, 5000)
            loss_chamfer = chamfer_distance_naive_batchwise(sampled_pred, sampled_target)

            loss_normal = normal_loss(pred_vert_pial, target_vert_pial, init_face)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--surface', default='pial', type=str,
    ...
    if hemi == 'left':
        ...
        if surface == 'wm':
            ...
            print('model = nn_surf_right_pial')
        print('# ============ 加载对应模板Finished =============')
