...
def train_loop(args):
    ...
    logging.basicConfig( 
    filename='/root/autodl-tmp/hcp1200/surface/ckpts_all_t2_repeat_soft_5_1/log_hemi-' + surf_hemi + '_' + 
             surf_type + '_' + tag + '.log',
    level=logging.INFO,
    format='%(asctime)s %(message)s')
    ...
    for epoch in tqdm(range(n_epoch+1)):
        ...
        if epoch % 10 == 0:  # start validation
            ...
            torch.save(nn_surf.state_dict(),
                       '/root/autodl-tmp/hcp1200/surface/ckpts_all_t2_repeat_soft_5_1/model_hemi-'+surf_hemi+'_'+\
