...
def train_loop(args):
    ...
    logging.basicConfig( 
    filename='/root/autodl-tmp/hcp1200/surface/ckpts_all_multi_repeat_5_1/log_hemi-' + surf_hemi + '_' + 
             surf_type + '_' + tag + '.log',
    level=logging.INFO,
    format='%(asctime)s %(message)s')
    ...
    for epoch in tqdm(range(n_epoch+1)):
        ...
        for idx, data in enumerate(trainloader):
            ...
            # print('repeat_loss:',repeat_loss)
            # match_counts = torch.tensor(match_counts, device=vert_gt.device)
            # repeated_matches = (match_counts > 1).float().sum(dim=1)  # shape: [B]

            # 可选：取 batch 中的平均作为惩罚项（你也可以选择求和）
            # repeat_loss = repeated_matches.mean()
        ...
        if epoch % 10 == 0:  # start validation
            ...
            torch.save(nn_surf.state_dict(),
                       '/root/autodl-tmp/hcp1200/surface/ckpts_all_multi_repeat_5_1/model_hemi-'+surf_hemi+'_'+\
...
if __name__ == "__main__":
    ...
    parser.add_argument('--w_repeat', default=0.0001, type=float, help="weight for edge length loss") # TODO # 增强边缘保持（成人皮质更薄）
