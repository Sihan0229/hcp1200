...
parser.add_argument('--nn_surf_left_wm_path', default='./surface/model/model_hemi-left_wm.pt', type=str,
                    help='Path of nn_surf_left_wm.')
parser.add_argument('--nn_surf_right_wm_path', default='./surface/model/model_hemi-right_wm.pt', type=str,
                    help='Path of nn_surf_right_wm.')
parser.add_argument('--nn_surf_left_pial_path', default='./surface/model/model_hemi-left_pial.pt', type=str,
                    help='Path of nn_surf_left_wm.')
parser.add_argument('--nn_surf_right_pial_path', default='./surface/model/model_hemi-right_pial.pt', type=str,
                    help='Path of nn_surf_right_pial.')
...
parser.add_argument('--save_suffix', default='_pial.surf.gii', type=str,
                    help='Suffix of save file.')
...
nn_surf_left_wm_path = args.nn_surf_left_wm_path
nn_surf_right_wm_path = args.nn_surf_right_wm_path
nn_surf_left_pial_path = args.nn_surf_left_pial_path
nn_surf_right_pial_path = args.nn_surf_right_pial_path
...
nn_surf_left_wm.load_state_dict(
    torch.load(nn_surf_left_wm_path, map_location=device))
nn_surf_right_wm.load_state_dict(
    torch.load(nn_surf_right_wm_path, map_location=device))
nn_surf_left_pial.load_state_dict(
    torch.load(nn_surf_left_pial_path, map_location=device))
nn_surf_right_pial.load_state_dict(
    torch.load(nn_surf_right_pial_path, map_location=device))
...
if __name__ == '__main__':
    ...
    for subj_t2_dir in tqdm(subj_list):
        ...
        # ============ Surface Reconstruction ============
        logger.info('----------------------------------------')
        logger.info('Surface reconstruction ({}) starts ...'.format(hemi))
        ...
        #     save_dir=subj_out_dir+'_hemi-'+hemi+'_wm_50-cd-nc-edge-100.surf.gii',#
        #     surf_hemi=hemi, surf_type='wm')
        save_gifti_surface(
            vert_pial_orig, face_orig, 
            save_dir=subj_out_dir+'_hemi-'+hemi+save_suffix,
            surf_hemi=hemi, surf_type='pial')
        # save_gifti_surface(
        #     vert_mid_orig, face_orig, 
        #     save_dir=subj_out_dir+'_hemi-'+hemi+'_midthickness.surf.gii',
        #     surf_hemi=hemi, surf_type='midthickness')
