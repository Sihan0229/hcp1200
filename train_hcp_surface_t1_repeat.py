...
class SurfDataset(Dataset):
    ...
    def __init__(self, args, data_split='train'):
        ...
        if self.surf_type == 'pial':
            ...
            model_path = f'/root/autodl-tmp/hcp1200/surface/model/model_hemi-left_wm_0001_240epochs.pt'
    ...
    def __getitem__(self, idx):
        ...
        # ------ 惰性加载体积数据（T1w）------
        vol_in = nib.load(os.path.join(subj_dir, 'T1w_proc_affine.nii.gz'))
        ...
        # pial表面需要预测
        if self.surf_type == 'pial':
            vert_in = torch.Tensor(vert_in[None])
            face_in = torch.LongTensor(face_in[None])
            vol_tensor = torch.Tensor(vol_data[None])
...
def train_loop(args):
    ...
    logging.basicConfig( 
    filename='/root/autodl-tmp/hcp1200/surface/ckpts_all_t1_repeat_soft_5_1/log_hemi-' + surf_hemi + '_' + 
             surf_type + '_' + tag + '.log',
    level=logging.INFO,
    format='%(asctime)s %(message)s')
    ...
    for epoch in tqdm(range(n_epoch+1)):
        ...
        if epoch % 10 == 0:  # start validation
            ...
            torch.save(nn_surf.state_dict(),
                       '/root/autodl-tmp/hcp1200/surface/ckpts_all_t1_repeat_soft_5_1/model_hemi-'+surf_hemi+'_'+\
